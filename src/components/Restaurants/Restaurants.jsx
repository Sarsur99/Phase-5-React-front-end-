...
const Restaurants = () => {
  ...
  const handleDelete = (slug) => {
    axios.delete(`http://localhost:3000/api/v1/restaurants/${slug}`)
      .then(response => {
        setRestaurants(restaurants.filter(rest => rest.attributes.slug !== slug));
      })
      .catch(error => {
        console.log(error);
      });
    ...
    return (
      <div key={rest.attributes.name} className='w-full bg-gradient-to-r from-pink-500 to-red-500' style={{ width: '100%' }}>
        <Restaurant
          ...
          id={rest.id}
          handleDelete={() => handleDelete(rest.attributes.slug)}
        />
      </div>
    );
  });
